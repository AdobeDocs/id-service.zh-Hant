---
description: 瀏覽器使用跨原始資源共用 (CORS) 來從目前網域以外的其他網域要求資源。 訪客ID服務支援CORS標準，以允許這些使用者端的跨原始資源要求。 訪客ID服務會在舊版瀏覽器或不支援CORS的瀏覽器上回覆為JSONP請求。
keywords: 訪客 ID 服務
title: Adobe訪客ID服務的CORS支援
...
# Adobe訪客ID服務的CORS支援 {#cors-support-in-the-experience-cloud-id-service}

瀏覽器使用跨原始資源共用 (CORS) 來從目前網域以外的其他網域要求資源。 訪客ID服務支援CORS標準，以允許這些使用者端的跨原始資源要求。 訪客ID服務會在舊版瀏覽器或不支援CORS的瀏覽器上回覆為JSONP請求。

## 相同來源政策與訪客ID服務請求的問題 {#section-6608cf46d27143eeaeabacaa6aa14e8e}
...
讓我們來看看這些標題如何運作。 在此範例中，假設有一家金融服務公司已在其網站上實作訪客ID服務， `www.finance-website.com`。 下表定義了 CORS 要求和回應標頭如何檢查對資源的存取權。
...
<table id="table_B004ACF52B5A4D33B1DCF7EA77BE4E6D"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>當金融公司頁面載入時，瀏覽器會向 <span class="codeph">dpm.demdex.net</span> 提出要求。 這是對訪客ID服務所使用的資料收集伺服器(DCS)網域的呼叫。 這個跨網域要求包含此標頭： </p> <p> 
     <ul class="simplelist"> 
