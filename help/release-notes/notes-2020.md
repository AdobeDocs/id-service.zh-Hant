...
* 預設為 `loadSSL` 標幟開啟。 所有對訪客ID服務的呼叫預設都會在`https`進行。  如果客戶想從`non-ssl`頁面透過http呼叫訪客ID服務，可將其設為false。
* 更新偵測 `Internet-Explorer (IE)` 版本的函數，以修正 `ESLint` 回報的問題。修正 ECID 收到選擇加入 `Internet-Explorer (IE) 11` 及稍後更新時 `pre-approval` 的效能問題。
...
如需所有產品的每月發行說明，請參閱[CX Enterprise發行說明](https://experienceleague.adobe.com/docs/release-notes/experience-cloud/current.html?lang=zh-Hant)。
