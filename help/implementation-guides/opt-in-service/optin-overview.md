...
>[!IMPORTANT]
>
>本文件的內容不是法律建議，且用意並非要取代法律建議。在設定您的選擇實作時，請洽詢您公司的法律部門，以取得有關同意和慣例的建議。

## 跨Experience Cloud解決方案選擇加入 {#section-053e6224505542cf961896f0ca869e52}
...
* 指出用戶是否一般適用許可收集要求。
* 指定允許產生Cookie的解決方案。
* 針對使用者未明確同意或拒絕類別的任何解決方案套用預設偏好設定。
* 根據使用者同意設定的變更觸發自訂回應，讓您保留或更新使用者設定。

您可以使用選擇加入服務設定網站，允許在使用者選擇前，讓部分 Cookie 載入預先同意。您可以為新客戶設定選擇加入服務，以便在使用者同意後或選項開放使用後載入 Cookie。您也可以從現有的「同意管理平台」儲存和擷取選擇加入的同意，或只是將選擇加入權限儲存在Cookie中。

![](assets/Opt-in-approval.png)

然後，Adobe解決方案可以檢查標籤是否獲得核准、訂閱變更，然後擷取所有選擇加入的客戶。 選擇加入服務可讓您直接透過解決方案的 JavaScript 資料庫或 ECID (若已實作) 取得權限。
