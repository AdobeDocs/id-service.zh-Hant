...
說明[!UICONTROL 組織 ID]、[!UICONTROL 一般]和[!UICONTROL 客戶設定]欄位，以及 [!DNL Experience Cloud] ID 服務如何使用這些欄位。
...
您在動態標籤管理 (DTM) 中新增並儲存 ID 服務做為工具後，就能使用這些設定。您也可透過按一下 DTM Web 屬性之[!UICONTROL 「已安裝工具」]區段中的齒輪圖示，來存取這些設定。
