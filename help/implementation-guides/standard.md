...
## 使用DTM實作Experience Platform Identity Service {#task-a659cf19dea84ad48edabe0b72ef9f5c}
...
1. 在 **[!UICONTROL 工具類型]** 清單中，按一下 **[!UICONTROL Experience Platform Identity Service]**。

   >[!NOTE]
   >
   >此動作會以您的組織ID填入 **[!UICONTROL Experience Cloud組織ID]** 方塊。如果 DTM 帳戶未連結至 [!DNL Experience Cloud]，您必須提供此 ID。若要連結帳戶，請參閱[在 Experience Cloud 中連結帳戶](https://marketing.adobe.com/resources/help/en_US/mcloud/organizations.html)。如需如何找到組織 ID 的相關資訊，請參閱[需求](../reference/requirements.md#section-a02f537129a64ffbb690d5738d360c26)。

1. 在 **[!UICONTROL 追蹤伺服器]** 方塊中鍵入追蹤伺服器的名稱。如果您不確定如何找到追蹤伺服器，請參閱 [常見問題](../faq-intro/faq.md) 並 [正確填入trackingServer和trackingServerSecure變數](https://helpx.adobe.com/analytics/kb/determining-data-center.html#)。
...
勾選時，動態標籤管理會在載入使用Experience Platform Identity Service的Adobe解決方案之前，自動呼叫 `getMarketingCloudVisitorID()` 方法。
...
可根據訪客的驗證狀態 (例如，登入、登出) 來識別訪客的選項。請參閱 [客戶ID和驗證狀態](../reference/authenticated-state.md)。

## 測試並驗證Experience Platform Identity Service {#concept-644fdbef433b46ba9c0634ac95eaa680}
